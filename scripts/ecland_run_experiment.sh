...
#   └── scripts/
#
# Vendored unmodified (engine logic only, comments/examples updated) from
# plumber2-ecland's scripts/ecland_run_experiment.sh: GROUP and FORCING_TYPE
# are already generic, so this runs any site group, not just PLUMBER2's.
#
...
Required experiment options:
  -g GROUP               Site group, for example CCI_LAKES
  -t FORCING_TYPE        Forcing filename pattern, for example era5HT
...
NAMELIST_DEFAULT="${PROJECT_ROOT}/namelists/namelist_ecland_lake_ctl"
